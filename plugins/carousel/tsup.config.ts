...
// Combined esbuild loader (mirrors quartz-community/explorer): transpiles
// `*.inline.ts` to a minified browser-JS string imported as text (used by the
// carousel client script). Also compiles `*.scss` if present.
...
export default defineConfig({
  ...
  format: ["esm"],
  // dts disabled: the `*.inline.ts` import is loaded as a string (no type surface),
  // which would break declaration generation. Runtime JS is unaffected.
  dts: false,
  sourcemap: true,
  clean: true,
  treeshake: true,
